...
export class Foam {
	...
	private initParticles() {
		...
		for (let i = 0; i < this.particleCount; i++) {
			const x = (Math.random() - 0.5) * seaSize.x;
			const z = (Math.random() - 0.5) * seaSize.y;
		...
		for (let i = 0; i < this.particleCount; i++) {
			...
			for (let dx = -1; dx <= 1; dx++) {
				for (let dz = -1; dz <= 1; dz++) {
					...
					if (
						neighborX >= 0 &&
						neighborX < weightMask.length &&
						neighborZ >= 0 &&
						neighborZ < weightMask[0].length
					) {
						...
						const heightDiff =
							...
							this.seaRef.getHeightAt(neighborPos);
			...
			// Apply displacement
			this.positions[i * 3] = lerp(
				this.positions[i * 3],
				this.initialPositions[i * 3] + displacementX,
				0.01
			);
			this.positions[i * 3 + 2] = lerp(
				this.positions[i * 3 + 2],
				this.initialPositions[i * 3 + 2] + displacementZ,
				0.01
			);

			// Update Y position based on sea height
			const newY = this.seaRef.getHeightAt(
				new THREE.Vector3(this.positions[i * 3], 0, this.positions[i * 3 + 2])
			);
