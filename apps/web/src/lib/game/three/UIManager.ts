...
export class UIManager {
	...
	constructor(
		...
		// Initialize plane
		this.plane = new Mesh(
			new PlaneGeometry(this.size.x, this.size.y)
				.rotateX(-Math.PI * 0.5)
				.translate(this.size.x / 2, 0, this.size.y / 2),
			new MeshBasicMaterial({ visible: false })
		);
		this.scene.add(this.plane);

		// Initialize gizmo
		this.gizmo = new Gizmo(GIZMO.SIZE, new Color(0xffffff), new Color(0xff0000));
	...
	private findPlayerShip(): Object3D | null {
		if (this.playerPubKeyBase58) return this.scene.getObjectByName(this.playerPubKeyBase58) || null;
		return null;
	}
	...
	public updateCameraPosition() {
		...
		if (playerShip) {
			...
			this.camera.position.copy(playerPosition).add(CAMERA.OFFSET);

			// Update camera look at
			const lookAtPosition = playerPosition.clone().add(CAMERA.LOOK_AT_OFFSET);
