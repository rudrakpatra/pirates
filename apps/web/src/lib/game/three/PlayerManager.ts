...
export class PlayerManager {
	private players: Map<string, Pirates.Proxy.ProxyPlayer> = new Map();
	...
	syncState(players: { [key: string]: Pirates.Proxy.ProxyPlayer }) {
	...
	private addPlayer(pubKey: string, playerData: Pirates.Proxy.ProxyPlayer) {
		if (this.shipModel) {
			const shipInstance = this.shipModel.clone();
			shipInstance.name = pubKey;

			shipInstance.position.set(playerData.ship.circle.x, 0, playerData.ship.circle.y);
	...
	private updatePlayer(pubKey: string, playerData: Pirates.Proxy.ProxyPlayer) {
	...
	private updateCannonballs() {
		for (const [pubKey, player] of this.players) {
			...
			if (ship && player.prevCannonBall) {
				...
				if (
					currentBlockHeight ===
					player.prevCannonBall.spawnBlockHeight + Pirates.Constants.CANNON_WAIT_TIME
				) {
	...
	private fireCannonball(ship: Object3D, player: Pirates.Proxy.ProxyPlayer) {
		if (player.prevCannonBall) {
			const explosionPosition = ship.position
				.clone()
				.add(new Vector3(player.prevCannonBall.circle.x, 0, player.prevCannonBall.circle.y));
	...
	getPlayer(pubKey: string): Pirates.Proxy.ProxyPlayer | undefined {
