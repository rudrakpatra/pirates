...
export const loadProxy = () => {
	...
	boatGroup.scale.set(SHIP.SIZE, SHIP.SIZE, SHIP.SIZE);
	boatGroup.updateMatrix();

	// Create a cuboid for loot
	const loot = new Mesh(new BoxGeometry(1, 1, 1), new MeshStandardMaterial({ color: 0xae8321 }));
	loot.name = 'loot';
	loot.userData.type = 'loot';
	loot.scale.set(LOOT.SIZE, LOOT.SIZE, LOOT.SIZE);
	...
	cannonball.scale.set(CANNON.SIZE, CANNON.SIZE, CANNON.SIZE);
